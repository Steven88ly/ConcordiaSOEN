		...
		try
		{
			...
			if (argv.length > 0)
			{
				try
				{
					...
					// Ensure the number of philosophers is a positive integer
					if (iPhilosophers <= 0)
					{
						throw new NumberFormatException("Number of philosophers must be a positive integer.");
					}
				}
			...
			// Main waits for all its children to finish
			for (int j = 0; j < iPhilosophers; j++)
			{
